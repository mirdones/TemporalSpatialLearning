...
if freedom == 2:
    ...
    model_name = '6_directions'

patience = 10


def getReport(generator):
    ...
    for i in tqdm.tqdm(range(len(generator))):
        batch_X, batch_y_true = generator.__getitem__(i)
    ...
    confusionMatrix = confusion_matrix(y_true_indices, y_pred_indices)
    print(f'\n{confusionMatrix=}\n')
...
def get_trained_model():
    ...
    print(f'\nval_generator: {len(val_generator)} items\n')

    train_generator = DataGenerator(mode=TRAIN, freedom=freedom)

    print(f'\ntrain_generator: {len(train_generator)} items\n')

    model = getModel(decay_steps=1 * len(train_generator), categories=freedom,
